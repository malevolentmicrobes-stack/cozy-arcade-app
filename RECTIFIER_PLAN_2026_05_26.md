...
**2026-07-07 Shadow Dungeon scheduled-only follow-up:** browser probe showed the overlay's `due` Shadow Dungeon path was still strict scheduled-only because `shadowScope351='due'` synced the global launch scope to `getStudyPool('due')`; after scheduled/repair cards were exhausted it had no pinned/random continuation. Fixed only the Shadow overlay path by finally populating the already-existing `__shadowRunQueue`: for `due`, queue is scheduled/relearning first, then review/pinned/hard, then shuffled random-new cards; for `review`, queue is review first, then due, then random-new fallback. Global `due` mode remains strict. Browser-validated sequence: `due_card` → `hard_card` → `pinned_card` → shuffled `new_*`; `Again`/`Hard`/`Good`/`Easy` all persisted through `advance()` with no Good override or double count.
...
**2026-07-07 final verification — 6-card state matrix, no new app edits.** Separate Codex pass seeded again/hard/pinned/due/new×2 card states and ran every mode above plus both Shadow queue orders through the real browser: `due` → due_card,again_card,hard_card; `review_deck` → pinned_card,again_card,hard_card; `pinned` → pinned_card only; `hard` → again_card,hard_card (not instantly repeated in an active random-new run, but present in Review Deck — correct semantics, not a bug); Shadow `due` sequence → due_card→again_card→hard_card→pinned_card→shuffled new. Also confirmed via real button-click + canonical Phase3 JSON export + public `window.cozyPhase3.importObjectPhase3(...)` reimport that `last_rating:'again'`/`stage:'relearning'` round-trips correctly and the card still leads the pool afterward. `runCozySmokeTests()` 6/6, zero page errors, `git diff --check` clean throughout — this closes the multi-round trace above. Consolidated reference for the full verified algorithm (all modes, both Shadow orders, rating-impact table) now lives in `COZY_ARCADE_PROJECT_STATUS_2026-07-07.md`. Still outstanding: user's own hands-on confirmation, and reconciling PHASE2's separate unreconciled fix for this same bug.

---

## 2026-07-07 continued — real FSRS formula bug found (missing "+1"), fixed, `runFSRSValidation()`'s own expected constants had been silently calibrated to match the bug

**User asked, given all the pool-ordering work above, whether the Good/Easy rating itself matches real Anki/FSRS.** Audited every FSRS function (`fsrsInitStability`, `fsrsInitDifficulty`, `fsrsRetrievability`, `fsrsForgetStability`, `fsrsUpdateDifficulty`, `fsrsRecallStability`) against the actual published FSRS algorithm by hand, then verified every hand-computation with a Python script (no node available) rather than trusting arithmetic alone.

**Four of the five formulas are correct**, verified against the real spec and the project's own test constants exactly. **`fsrsRecallStability` — the Hard/Good/Easy stability-growth formula — was missing the "+1" the real formula requires.** Real FSRS: `S' = S × (1 + [growth term])`. This file had: `S' = max(S, S × [growth term])` — using the growth term as a *replacement* for S rather than an *addition* to it. Precise computation (D=5.3146, S=3.1262, R=0.9038, rating=Good): real formula gives S'≈10.063; this file's old formula gave S'≈6.937 — **which is exactly the hardcoded value (`6.9371`) `runFSRSValidation()`'s own `'S good->good'` check expected.** The test had been calibrated to match this bug's output, not the real algorithm — so "FSRS 17/17" has been passing this whole project's history without ever validating against true FSRS.

**Confirmed this bug affects all three games identically** — Solo, Domain, and Shadow Dungeon all funnel every rating through the same single `rateCard()` → `fsrsRecallStability()` call, no per-mode divergence, so no separate per-game fix was needed.

**Confirmed "pin" has zero interaction with any of this**, exactly matching the user's own framing (a simple tag, not a rating): `rateCard()`'s `rating==='pin'` branch toggles `pinned` and returns immediately, before any FSRS math runs at all.

**Fixed:** added the missing `(1 + ...)` to `fsrsRecallStability`. Recalibrated `runFSRSValidation()`'s two dependent constants using precise Python computation, not hand-rounding: `'S good->good'` `6.9371→10.0633`, `'int good->good'` `7→10`. The `'easy bonus'` boolean check (`Se > 2×S0`) needed no change — the corrected value (≈9.58) still clears that bar, more robustly than before. `fsrsForgetStability` (Again/lapse) and `fsrsUpdateDifficulty` were independently re-verified correct and untouched.

**Worth remembering for anything touching this test suite again:** a passing internal validation test is only proof of self-consistency, not correctness against the real external spec it claims to check — this is the same shape of lesson as the pool-filter bugs above (a test/guard can be quietly calibrated to match an implementation's actual behavior rather than the behavior it's supposed to enforce), just found in the core scheduling algorithm instead of a UI filter this time.

**Browser validation completed by Codex after Claude's formula patch:** real Chrome run against this repo returned `runFSRSValidation()` 17/17 and `runCozySmokeTests()` 6/6. Focused reviewed-card probe confirmed Good now schedules 10d with stability ≈10.063, Easy schedules 22d and larger stability than Good, Hard remains `repair_point:true`, Again remains `stage:'relearning'`/interval 0, and Pin toggles `pinned:true` without mutating `last_rating`, stage, interval, stability, difficulty, or review count.
