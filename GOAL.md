...
## UPDATE 2026-07-06 — SM2 "Again" card doesn't resurface, root cause traced across BOTH game modes (not coded yet, premortem + plan only, per standing rule)

**User's report:** in Solo Studying, rating a card "Again" never brings it back within ~10 minutes as expected. Same symptom reported in Knowledge Expansion (Domain) too — "not seeing it in either."

**Root cause #1 — the actual filter gap, confirmed by reading `getStudyPool()` (line ~11590):** `isReviewedCard(card)` returns true the instant a card has any `last_rating` or `stage:'relearning'` — i.e. immediately after the first "Again." `isNewCard(card)` is just `!isReviewedCard(card)`. Solo's factory default order, `solo_order:'random_new'` (line ~10984), filters to `arr.filter(isNewCard)` only — so an Again-rated card is permanently excluded from that pool for the rest of the session, not just for 10 minutes. This is not an SM2 math bug: `previewInterval()` correctly schedules Again as a 10-minute relearning step. It's the pool filter silently hiding a card that IS correctly scheduled.

**Root cause #2 — why Domain broke too, even though its own default (`expansion_order:'new_first'`) already handles this correctly** (its filter explicitly re-includes not-yet-due relearning cards, unlike `random_new`): traced `resetStudyFiltersForImportedDeck()`, added yesterday inside Codex's authoritative import gate, and the two empty-pool safety-net fallbacks added yesterday in the Solo/Phase3 start wrappers. All three, when triggered (which yesterday's fresh 1249-card ABIM import did trigger), set **both** `phase3State.settings.solo_order` **and** `.expansion_order` to `'random_new'` — one of them explicitly via `syncGeneralStudyScopePhase3('random_new',{syncExpansion:true})`. So yesterday's emergency "no cards match filter" recovery silently overwrote Domain's normally-correct default into the one mode that has the same exclusion gap as Solo. This is a real side effect of yesterday's crisis fix, not a new independent bug — and it explains the user seeing the identical symptom in both game types starting the same day.

**Confirmed the standing "5-6 duplicate scope filters" item (flagged above, never actioned) is mostly dead code, not a live multi-owner conflict:** the `scope==='due'`/`deckMode==='due'` implementations at the old line numbers (1396, 3262, 4338, 4482, 6166, plus `cardPool()` at 394) are all legacy pre-Phase3 pool builders that only *sort* by due-score, never filter anything out — and per the already-validated E7 fix (2026-06-03), Phase3's `getStudyPool()`/`sessionPool()` is confirmed the live pool for both Solo and Domain (`window.cardPool`/`window.getStudyPool` both point here). The real, live gap is entirely inside `getStudyPool()`'s own per-mode branches, not a competing legacy implementation winning.

**Proposed fix (not coded — premortem+plan only, awaiting go-ahead per the standing SM2 rule above):**
1. In `getStudyPool()`, change `new_only` and `random_new` (and audit `reviewed_first`, which has a narrower version of the same gap — excludes not-yet-due relearning cards, only picks them up once actually due) to prepend due/relearning cards ahead of the new-card set, mirroring `new_first`'s already-correct, already-shipped pattern — so a card that's SM2-due wins regardless of which scope/order is currently selected.
2. Decide whether this should be applied per-branch (safer, smaller diff) or hoisted to one shared step at the top of `getStudyPool()` that runs before the per-mode branching (more robust against any 6th mode being added later without the same care, but a bigger structural change).
3. Separately, not blocking: whether the Home Screen's scope dropdown should be simplified/reduced. User says OK to leave the Shadow Dungeon setup modal's own scope picker untouched for now.

**FIXED, same day, after user's go-ahead on the shared-step approach.** Added `dueRelearningFirst(list)` helper right after `dueSort` inside `getStudyPool()` — one shared definition, not copy-pasted per branch. Applied to `new_only` (previously excluded due/relearning entirely — now `[...dueRelearningFirst(arr), ...arr.filter(isNewCard)]`), `random_new` (previously `shuffleCards(arr.filter(isNewCard))`, dropping due/relearning entirely — now due/relearning cards come first, unshuffled, then the new-card shuffle after), and `reviewed_first` (had the narrower gap — added the missing not-yet-due-relearning bucket between the due-sorted list and new cards). `new_first` (Domain's default, already correct) left untouched — its existing 3-bucket pattern is what the helper mirrors. Deliberately did **not** touch `pinned`/`hard`/`tagged`/`suspended`/`due`/`review_deck` — those are explicit, exclusive user selections (or already correctly inclusive, in `review_deck`'s case) where injecting unrelated due cards would violate the user's own narrow choice, not fix a gap.

**Also verified, not just assumed, that the existing `seenThisSession` session-block wouldn't silently undo this fix:** the rating handler already does `session.seenThisSession.delete(cardId)` specifically on an 'again' rating (line ~11458, pre-existing code, not written today) — so the just-answered card is already exempted from the "don't show the same card twice in one sitting" block before `getStudyPool` ever runs again. Confirmed `sessionPool()`'s `poolKey` includes `session.seenThisSession.size`, so deleting from that set changes the key and forces a real recompute (not a stale cached pool) on the very next pool check. The two mechanisms compose correctly.

**Not live-browser-validated** — Chrome extension was disconnected all session; validated by direct source trace (isDue/isReviewedCard/isNewCard semantics, poolKey invalidation, splice-vs-recompute ordering) rather than an actual answer→Again→resurface run. Ask the user to confirm live before considering this fully closed. Full technical trace also logged in `RECTIFIER_PLAN_2026_05_26.md`.

**UPDATE same day — repo mix-up found, two more real gaps in this repo caught and fixed by re-checking priority, not just inclusion:** a separate Codex run fixed this same bug class, but in `cozy-arcade-app-PHASE2`, not this repo — worth flagging since two agents were independently working "the SM2 bug" in different repos. Re-tracing PHASE2's real findings against this file's actual code found two things the earlier pass here got wrong or missed: (1) `new_first` (Domain's default) was claimed "already correct" above — true for *inclusion*, false for *priority*: its old bucket order put new cards before due/relearning cards, so with ~1249 new cards ahead of it, the just-failed card still wasn't shown next. Fixed to lead with `dueRelearningFirst()` like the other modes. (2) `due` mode strictly required `isDue()`, excluding a card that's `relearning` but not yet at its `next_due_at` — the same shape of gap that caused the original two-days-ago crisis. Fixed both the primary and retry-path `due` branches to also accept `stage === 'relearning'`. Also confirmed `__shadowRunQueue` (Shadow Dungeon's dedicated queue, checked first in `nextCard()`) is never actually populated anywhere in this file — that branch is dead, so Shadow Dungeon runs through the same `getStudyPool()` path fixed here, including the `due` fix. Added a defensive explicit `session.poolKey=''; index=0;` in the 'again' handler too, matching what Codex's PHASE2 fix needed (this repo's `poolKey` already includes `seenThisSession.size` so it should have been redundant, but cheap insurance). Still not live-tested in this repo specifically.

**UPDATE 2026-07-07 — Codex reviewed Claude's PHASE1 patch and browser-tested this repo directly.** Solo `Again` resurfaces the same card, Knowledge Expansion `Again` resurfaces the same card, and Shadow Dungeon launches on `due`, `random-new`, and `review` scopes in a controlled relearning state. Also tightened the PHASE1 code to match the broader PHASE2 predicate (`relearning`/`repair_point`/`rating:again`/`last_rating:again`), cleared `buriedToday` on `Again`, and removed the misleading inline claim that `due` was not touched.

---

## UPDATE 2026-07-05 continued — Clear Local State fixed for real, Shadow Dungeon text simplified, two premortems for future goals (nothing else coded)

**Clear Local State was broken for 3 weeks and nobody knew — fixed.** Every "Clear Local State" button in the app (there were two, on different screens) only reset `soloStudyingState_v1757` — a key that stopped being written to entirely on 2026-06-15 (a comment in the code already said so, just never connected to this button). The actual canonical FSRS progress lives in `cozy_arcade_progress_v1` via `phase3State.progress`, which the button never touched. Clicking it did nothing to real progress — this is exactly why manually clearing cookies/browser history was the only thing that ever worked. Fixed with one shared `window.clearAllLocalProgress()` that clears the real store through the canonical `savePhase3State()` path; both old buttons now call it and both gained a confirmation step (native `confirm()` — previously neither had ANY confirmation, so this is a second bug fixed for free). Also added a new "Advanced Settings" collapsible section to the Settings & Gameplay drawer with a "Clear Local State" button and a custom inline Accept/Cancel confirm (matching the drawer's own visual style rather than a native dialog). Live-validated: Cancel leaves progress untouched, Accept drops progress from 3 cards to 0 in both `phase3State.progress` and the raw `cozy_arcade_progress_v1` localStorage key, normal progress persistence across a hard reload re-confirmed unaffected, FSRS 17/17, smoke 6/6, zero errors.

**Shadow Dungeon modal description text simplified** per exact wording requested, one string replacement, no logic touched.

**Premortem 1 (not coded) — Shadow Dungeon "extend card review timing before auto-advance":** traced `shadowDropSeconds351()` — it's a one-line function that returns `questionSeconds351()` unconditionally, i.e. **the Shadow Dungeon setup modal's "Game timing" dropdown is non-functional**, exactly like the dead `shadowSchedule351` dropdown already removed this session. Selecting a different value in that dropdown does nothing; the actual per-card timer during Shadow Dungeon play is always whatever Solo Studying's shared timer is set to. No comment or history explains this as an intentional past workaround — it reads as simply never wired up. The user's own framing ("multiple patches/glitches and subsequent card state glitches") suggests past Shadow Dungeon instability, but the documented history of that instability (GOAL.md's E5 entry, 2026-06-03) was about card **pool/selection** logic, not the timer — so there's no found evidence this specific delegation was a deliberate fix for a timer-related bug. Still flagged as not-yet-safe to touch reactively: any change here needs a full regression pass of the Shadow Dungeon answer→reveal→advance cycle specifically (not just Solo's), given how many duplicate scope-filter implementations already exist nearby that could plausibly interact with timing.

**Premortem 2 (not coded) — "System Surge"/"Weak System Detected" integration, user's stated future goal, explicitly "very delicate":** traced the exact screenshot ("WEAK SYSTEM DETECTED — ENDO energy unstable...A 5-card stabilization sequence is ready.") to `checkKnowledgePulse()` (line ~5990) calling a **bare** `showPulseToast(KNOWLEDGE_PULSE.twentyFive, ..., 'gold')` at line 6000 — this resolves to the same `.pulseToast351` bottom-toast system documented earlier this session (bare calls ≠ `window.showPulseToast`), confirmed by the gold border/title styling matching `.pulseToast351.gold` CSS exactly. Separately, `vEnergyPulse` (today's neural-pulse-adjacent script, ~line 14493) **already has its own message tables keyed `'SYSTEM SURGE'` and `'WEAK SYSTEM DETECTED'`** — meaning there is already partial, likely-unintentional groundwork for exactly the consolidation the user wants, sitting unused in a second system that never gets called for this trigger. Any future integration touches at minimum three systems simultaneously: `.pulseToast351` (persona milestone banners, currently live for this), `vEnergyPulse`/`.energyChip_v` (rating feedback, has unused message tables for the same categories), and `neuralPulse371` (today's top-of-screen status line, the ✻ format the user wants as the eventual single home). This needs a dedicated design decision — which system becomes canonical and which get deleted — not a reactive code change; nothing here was touched.
...
**Status:** PHASE1 commit `b20a1ef` fixed Shadow Dungeon progress sharing, data repair export, and ghost prevention; PHASE1 `sw.js` cache is `cozy-arcade-v40`. Cache-busted headless validation passed: `String(window.cardPool)` references Phase 3 `sessionPool`; `String(window.nextCard)` includes the Shadow Dungeon guard; `runFSRSValidation()` returns 17/17; `runCozySmokeTests()` returns 6/6.
